...
/*********************************************************************/
//求最大值和最小值
 //arr=[1,43,2,4,55,6,2,5];
 //arr1=[22,33,44,55,666,7,8];
/* alert(Math.max.apply(Array,arr));//求arr的最大值
 alert(Math.min.apply(Array,arr));*/
 Array.prototype.max=function(){//最大值得原型
  return Math.max.apply(Array,this);
 }
 Array.prototype.min=function(){
  return Math.min.apply(Array,this);
 }
 //var arr2=new Array(2,33,444,5,6,7,8);
 //alert(arr2.max());
 //求最大值和最小值
 /**********************************************************/
...
/*********************************************************************/
//获取具有定位属性的父元素相对于body    left  top值
//offset(obj).left      获取相对于body  left
//offset(obj).top       获取相对于body  top 
function offset(obj){
...
//鼠标拖拽效果函数封装
  function drag(obj){
     ...
     var ev=e||window.event;//获取事件对象的兼容
      ox=ev.offsetX||ev.layerX;//获取鼠标到有定位属性父容器的宽，有兼容问题
      oy=ev.offsetY||ev.layerY;//
      dx=ev.clientX;//对象到浏览器的宽度
      dy=ev.clientY;//对象到浏览器的高
      var offsets=offset(obj);
      var parentL=offsets.left;
      var parentT=offsets.top;
      stopClient(ev);
     document.onmousemove=function(e){
    var ev=e||window.event;
     dx=ev.clientX;//对象到浏览器的宽度
     dy=ev.clientY;//对象到浏览器的高
    obj.style.left=(dx-ox-parentL)+"px";//赋值属性
    obj.style.top=(dy-oy-parentT)+"px";//
     }
...
function tab(options){
    ...
    this.linkslen=options.links.length;
...
/*******************************************************************************/
//左右点击，滑动轮播
//tu:banner图   listli:轮播图下滑动的点  hot为listli中不一样的点
//left  right  左右键   box 包含整个图的大div   
...
//tu:banner图   listli:轮播图下滑动的点  hot为listli中不一样的点
//left  right  左右键   box 包含整个图的大div 
function huadong(tu,listli,left,right,box){
  var tuw=tu[0].offsetWidth;
  var len=tu.length;
  for(var i=0;i<len;i++){
    if(i!=0){
